    ...
    <div class="audio-container">
        ...
        >
            ...
            >
                ...
                @click="musicList_delete(i)"> 删除</button>
            </div>
            <div
...
import { reactive, watch, ref, type Ref, nextTick, onBeforeUnmount,onMounted } from 'vue';
import defaultCover from "@/assets/4399.png";
import { usePlayStore } from '@/stores/play';

const {play,musicList_delete} =usePlayStore()

watch(()=>play.musicList,()=>{
    //更新playview的歌词
    play.current_music_textArray=getTextArray(play.musicList[playState.index]?.lrc)
	    //更新当前歌曲
	play.current_music=play.musicList[playState.index]
})
watch(()=>play.play_index,(newVal)=>{
    if(playState.index!=newVal){
        playState.index=newVal
    }
...
watch(()=>play.play,(newVal)=>{
    if(newVal!=playState.play){
        if (playState.play) audio.value.pause();
        else audio.value.play();
        playState.play = !playState.play;
    }
...
watch(
    ...
    (newVal) => {
        ...
        play.play_index=playState.index
        //更新playview的歌词
	    play.current_music_textArray=getTextArray(play.musicList[playState.index]?.lrc)
	    //更新当前歌曲
	    play.current_music=play.musicList[playState.index]
