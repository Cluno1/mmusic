...
8. websocket
   ...
   6.   
   7.   1对多直播视频 MCU架构 混合所有用户视频音频流  b/s 方便管理
   8.   我需要在watch里面先更新组件的循环，等待dom树重新构建，再获取查询dom结点，用nexttick确保
   9.   只能在局域网，后续配置stun/turn ：用coturn做转发服务器
   10.  scroll 事件需要该元素设置了height！！！
   11.  v-if 如果img设置了v-if 他根本不会被挂到dom上面，也就不会触发load
   12.  一个循环播放可交互的盒子，需要用装饰器decorate的闭包
   13.  i18n
   14.  token 前后端唯一标识  后端根据token识别用户. 放在localStrorage
